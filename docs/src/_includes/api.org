...
curl -X GET http://star-api.herokuapp.com/api/v1/stars | python -m json.tool | head -n 24
#+END_SRC
...
*** Get a single star (like the Sun)

#+BEGIN_SRC sh :results output code :exports both
curl http://star-api.herokuapp.com/api/v1/stars/Sun | python -m json.tool | head -n 30
#+END_SRC
